...
// Converted from React Router Feed.jsx
//
// Changes:
//   1. "use client" directive
//   2. supabase import: ../../lib/supabase → @/lib/supabase
//   3. No useNavigate in original — no router change needed
//   4. window.confirm and window.location.origin are client-only — safe with "use client"
    ...
    <article className={`fd-post-card${post.is_pinned ? " fd-pinned" : ""}`}>
      ...
      <div className="fd-post-actions" role="group" aria-label="Post actions">
        <button className={`fd-action-btn${liked?" fd-liked":""}`} onClick={handleLike}
          ...
          <span className="fd-action-icon" aria-hidden="true">{liked ? "❤️" : "🤍"}</span>
...
function ComposeBox({ profile, userId, isAdmin, onPosted, repostOf = null, onCancelRepost }) {
  ...
  return (
    <div className="fd-compose" role="form" aria-label="Create a post">
      ...
      {repostOf && (
        <div className="fd-repost-preview">
          ...
          <div className="fd-rp-content">
            &ldquo;{repostOf.content?.slice(0,120)}{repostOf.content?.length > 120 ? "…" : ""}&rdquo;
          </div>
          <button className="fd-rp-cancel" onClick={onCancelRepost} aria-label="Cancel repost">✕ Cancel</button>
        </div>
      )}
      ...
      <div className="fd-compose-extras">
        <div className="fd-compose-tags">
          ...
          <label htmlFor="compose-location" style={{ position:"absolute", width:1, height:1, overflow:"hidden", clip:"rect(0,0,0,0)" }}>Location</label>
          <input
            id="compose-location"
        ...
        <div className="fd-compose-footer">
          <span className="fd-char-count" aria-live="polite">{content.length}/2000</span>
...
const CSS = `
  .fd-root{max-width:680px;margin:0 auto;font-family:'Inter',system-ui,sans-serif;padding-bottom:40px}
  .fd-feed-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:18px}
  .fd-feed-title{font-size:1.25rem;font-weight:900;color:#0f172a;letter-spacing:-.4px;margin:0}
  .fd-admin-badge{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;border-radius:100px;background:rgba(29,155,240,.1);border:1px solid rgba(29,155,240,.25);color:#0369a1;font-size:.78rem;font-weight:700}
  .fd-compose{background:rgba(255,255,255,.92);border:1px solid rgba(30,58,138,.1);border-radius:18px;padding:18px;margin-bottom:16px;box-shadow:4px 4px 16px rgba(15,23,42,.05),-4px -4px 12px rgba(255,255,255,.8)}
  .fd-compose-header{display:flex;align-items:center;gap:11px;margin-bottom:12px}
  .fd-compose-name{font-weight:700;font-size:.9rem;color:#0f172a;display:flex;align-items:center;gap:3px}
  .fd-compose-textarea{width:100%;padding:12px 14px;border-radius:13px;border:1.5px solid rgba(30,58,138,.1);background:#f8fafc;font-family:inherit;font-size:.92rem;color:#0f172a;line-height:1.55;resize:none;box-sizing:border-box;transition:all .2s}
  .fd-compose-textarea:focus{outline:none;background:#fff;border-color:rgba(59,130,246,.35);box-shadow:0 0 0 3px rgba(59,130,246,.07)}
  .fd-compose-error{font-size:.8rem;color:#dc2626;font-weight:600;margin:6px 0;padding:8px 12px;background:rgba(239,68,68,.07);border-radius:8px}
  .fd-compose-extras{margin-top:10px}
  .fd-compose-tags{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px}
  .fd-tag-btn{padding:6px 13px;border-radius:100px;border:1.5px solid rgba(30,58,138,.12);background:rgba(30,58,138,.04);color:#475569;font-size:.8rem;font-weight:600;cursor:pointer;transition:all .15s;white-space:nowrap;font-family:inherit}
  .fd-tag-btn:hover{background:rgba(30,58,138,.09)}
  .fd-feelings-wrap{position:relative}
  .fd-feelings-dropdown{position:absolute;top:calc(100% + 6px);left:0;z-index:100;background:#fff;border:1px solid rgba(30,58,138,.12);border-radius:14px;padding:8px;box-shadow:0 12px 32px rgba(15,23,42,.12);display:grid;grid-template-columns:1fr 1fr;gap:3px;min-width:220px}
  .fd-feeling-item{padding:7px 10px;border-radius:9px;border:none;background:transparent;text-align:left;font-size:.82rem;font-weight:600;color:#334155;cursor:pointer;transition:background .12s;font-family:inherit}
  .fd-feeling-item:hover{background:rgba(30,58,138,.07)}
  .fd-feeling-clear{color:#ef4444;grid-column:span 2}
  .fd-location-input{flex:1;min-width:120px;padding:6px 12px;border-radius:100px;border:1.5px solid rgba(30,58,138,.12);background:rgba(30,58,138,.04);font-size:.8rem;font-family:inherit;color:#475569;outline:none;transition:all .15s}
  .fd-location-input:focus{border-color:rgba(59,130,246,.3);background:#fff}
  .fd-compose-footer{display:flex;align-items:center;justify-content:space-between}
  .fd-char-count{font-size:.75rem;color:#94a3b8;font-weight:600}
  .fd-post-btn{padding:9px 24px;border-radius:100px;border:none;background:linear-gradient(135deg,#1e3a8a,#3b82f6);color:#fff;font-size:.88rem;font-weight:800;cursor:pointer;transition:all .15s;box-shadow:0 3px 0 #1e40af;font-family:inherit}
  ...
  .fd-repost-preview{background:rgba(59,130,246,.05);border:1.5px solid rgba(59,130,246,.15);border-radius:12px;padding:11px 14px;margin-bottom:10px;display:flex;align-items:flex-start;gap:10px;flex-wrap:wrap}
  .fd-rp-label{font-size:.78rem;font-weight:700;color:#3b82f6;white-space:nowrap}
  .fd-rp-content{flex:1;font-size:.84rem;color:#475569;font-style:italic;min-width:0}
  .fd-rp-cancel{font-size:.75rem;color:#ef4444;font-weight:700;border:none;background:none;cursor:pointer;white-space:nowrap;font-family:inherit}
  .fd-post-card{background:rgba(255,255,255,.92);border:1px solid rgba(30,58,138,.08);border-radius:18px;padding:18px;margin-bottom:14px;position:relative;box-shadow:4px 4px 16px rgba(15,23,42,.05),-4px -4px 12px rgba(255,255,255,.8);animation:fdCardIn .3s cubic-bezier(.16,1,.3,1) both}
  @keyframes fdCardIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
  .fd-pinned{border-color:rgba(245,158,11,.3);box-shadow:4px 4px 16px rgba(245,158,11,.08),-4px -4px 12px rgba(255,255,255,.8)}
  .fd-pin-badge{display:inline-block;margin-bottom:10px;padding:3px 10px;border-radius:100px;background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.25);font-size:.74rem;font-weight:700;color:#b45309}
  .fd-post-header{display:flex;align-items:flex-start;gap:11px;margin-bottom:12px}
  .fd-post-meta{flex:1;min-width:0}
  .fd-post-author{font-weight:800;font-size:.93rem;color:#0f172a;display:flex;align-items:center;gap:3px}
  .fd-post-info{font-size:.76rem;color:#94a3b8;font-weight:500;margin-top:2px;display:flex;flex-wrap:wrap;align-items:center;gap:3px}
  .fd-dot{color:#cbd5e1}
  .fd-post-menu-wrap,.fd-comment-menu-wrap{position:relative}
  .fd-dot-btn{background:none;border:none;cursor:pointer;color:#94a3b8;font-size:1.1rem;padding:4px 8px;border-radius:8px;transition:all .15s;line-height:1;font-family:inherit}
  .fd-dot-btn:hover{background:rgba(30,58,138,.07);color:#475569}
  .fd-dot-btn-lg{font-size:1.3rem;padding:4px 10px}
  .fd-dot-btn:focus-visible{outline:2px solid #3b82f6;outline-offset:2px}
  .fd-dropdown{position:absolute;top:calc(100% + 4px);right:0;z-index:50;background:#fff;border:1px solid rgba(30,58,138,.1);border-radius:13px;padding:6px;box-shadow:0 12px 32px rgba(15,23,42,.13);min-width:160px;animation:fdDropIn .18s ease}
  @keyframes fdDropIn{from{opacity:0;transform:scale(.95) translateY(-6px)}to{opacity:1;transform:none}}
  .fd-dropdown button{display:block;width:100%;text-align:left;padding:9px 13px;border:none;background:transparent;font-size:.83rem;font-weight:600;color:#334155;border-radius:9px;cursor:pointer;transition:background .12s;font-family:inherit}
  .fd-dropdown button:hover{background:rgba(30,58,138,.07)}
  .fd-repost-badge{font-size:.79rem;color:#64748b;margin-bottom:8px;padding:5px 10px;background:rgba(59,130,246,.05);border-radius:8px;border-left:3px solid #3b82f6}
  .fd-post-content{font-size:.95rem;color:#1e293b;line-height:1.65;white-space:pre-wrap;word-break:break-word;margin-bottom:14px}
  .fd-post-actions{display:flex;gap:2px;flex-wrap:wrap;padding-top:10px;border-top:1px solid rgba(30,58,138,.06)}
  .fd-action-btn{flex:1;display:flex;align-items:center;justify-content:center;gap:6px;padding:8px 6px;border:none;background:transparent;border-radius:10px;font-size:.8rem;font-weight:600;color:#64748b;cursor:pointer;transition:all .15s;white-space:nowrap;font-family:inherit}
  .fd-action-btn:hover{background:rgba(30,58,138,.06);color:#1e3a8a}
  .fd-action-btn:focus-visible{outline:2px solid #3b82f6;outline-offset:2px}
  .fd-liked{color:#e11d48}
  .fd-liked:hover{color:#e11d48}
  .fd-action-icon{font-size:1rem}
  .fd-comments-section{margin-top:12px;padding-top:12px;border-top:1px solid rgba(30,58,138,.06);display:flex;flex-direction:column;gap:10px}
  .fd-comment{display:flex;gap:9px;align-items:flex-start}
  .fd-comment-body{flex:1;min-width:0}
  .fd-comment-header{display:flex;align-items:center;gap:7px;flex-wrap:wrap}
  .fd-comment-name{font-weight:700;font-size:.83rem;color:#0f172a;display:flex;align-items:center;gap:3px}
  .fd-comment-time{font-size:.72rem;color:#94a3b8;font-weight:500}
  .fd-comment-text{font-size:.87rem;color:#334155;line-height:1.5;margin:3px 0 0;word-break:break-word}
  .fd-comment-input-row{margin-top:6px}
  .fd-comment-input-wrap{display:flex;gap:8px;align-items:center}
  .fd-comment-input{flex:1;padding:9px 14px;border-radius:100px;border:1.5px solid rgba(30,58,138,.12);background:#f8fafc;font-family:inherit;font-size:.86rem;color:#0f172a;outline:none;transition:all .15s}
  .fd-comment-input:focus{border-color:rgba(59,130,246,.3);background:#fff;box-shadow:0 0 0 3px rgba(59,130,246,.06)}
  .fd-comment-send{width:36px;height:36px;border-radius:50%;border:none;background:linear-gradient(135deg,#1e3a8a,#3b82f6);color:#fff;font-size:.85rem;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;flex-shrink:0;font-family:inherit}
  ...
  .fd-overlay{position:fixed;inset:0;z-index:9999;background:rgba(2,6,23,.55);display:flex;align-items:center;justify-content:center;padding:16px}
  .fd-modal{background:#fff;border-radius:22px;padding:28px 24px;max-width:420px;width:100%;box-shadow:0 30px 80px rgba(2,6,23,.2);animation:fdModalIn .3s cubic-bezier(.16,1,.3,1)}
  @keyframes fdModalIn{from{opacity:0;transform:scale(.9) translateY(16px)}to{opacity:1;transform:none}}
  .fd-modal-title{font-size:1.05rem;font-weight:900;color:#0f172a;margin-bottom:6px}
  .fd-modal-sub{font-size:.83rem;color:#64748b;margin-bottom:16px}
  .fd-report-reasons{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:14px}
  .fd-reason-btn{padding:7px 14px;border-radius:100px;border:1.5px solid rgba(30,58,138,.15);background:transparent;font-size:.8rem;font-weight:600;color:#475569;cursor:pointer;transition:all .15s;font-family:inherit}
  .fd-reason-btn.active,.fd-reason-btn:hover{background:rgba(30,58,138,.08);border-color:rgba(30,58,138,.3);color:#1e3a8a}
  .fd-modal-textarea{width:100%;padding:10px 13px;border-radius:12px;border:1.5px solid rgba(30,58,138,.12);background:#f8fafc;font-family:inherit;font-size:.86rem;resize:none;box-sizing:border-box;outline:none;margin-bottom:14px}
  .fd-modal-actions{display:flex;gap:9px;justify-content:flex-end}
  .fd-modal-cancel{padding:9px 20px;border-radius:11px;border:1.5px solid #e2e8f0;background:transparent;font-size:.86rem;font-weight:700;color:#64748b;cursor:pointer;font-family:inherit}
  .fd-modal-confirm{padding:9px 20px;border-radius:11px;border:none;font-size:.86rem;font-weight:800;cursor:pointer;transition:all .15s;font-family:inherit}
  .fd-modal-report{background:linear-gradient(135deg,#dc2626,#ef4444);color:#fff;box-shadow:0 3px 0 #b91c1c}
  .fd-modal-report:disabled{background:#cbd5e1;box-shadow:none;cursor:not-allowed}
  .fd-report-toast{position:absolute;top:12px;left:50%;transform:translateX(-50%);background:#15803d;color:#fff;padding:7px 16px;border-radius:100px;font-size:.8rem;font-weight:700;white-space:nowrap;z-index:10;animation:fdToastIn .3s ease}
  ...
  .fd-loading{display:flex;align-items:center;gap:10px;justify-content:center;padding:24px;color:#64748b;font-size:.85rem;font-weight:600}
  .fd-spinner{width:20px;height:20px;border-radius:50%;border:2.5px solid rgba(59,130,246,.2);border-top-color:#3b82f6;animation:fdSpin .7s linear infinite}
  @keyframes fdSpin{to{transform:rotate(360deg)}}
  .fd-empty{text-align:center;padding:40px 20px;color:#94a3b8;font-size:.9rem;font-weight:600}
  .fd-end{text-align:center;padding:20px;color:#cbd5e1;font-size:.8rem;font-weight:600}
