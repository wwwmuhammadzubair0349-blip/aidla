...
function buildPrintCss(paper) {
  ...
  return `
    /* ==========================================================
       1. SMART HARDWARE MARGINS
    ========================================================== */
    @page {
      size: ${mm_w}mm ${mm_h}mm;
      /* Top/Bottom margins applied to EVERY page to give breathing room */
      margin: 12mm 0mm; 
    }
    
    @page :first {
      /* Remove top margin ONLY on Page 1 so the blue header touches the top edge */
      margin-top: 0mm; 
    }
    ...
    /* ==========================================================
       3. PRESERVE ORIGINAL LAYOUT (Flex/Grid stays intact)
    ========================================================== */
    body.cv-printing .cv-doc {
      ...
      margin: 0 !important;
      padding: 0 8mm !important; /* Left/Right padding only */
    ...
    /* ==========================================================
       4. SURGICAL PAGE BREAK FIXES (Photo bug fixed!)
    ========================================================== */
    
    /* A. Content Blocks: These NEED the width:100% hack to prevent Chrome from slicing them in flexbox */
body.cv-printing .cv-item {
    ...
    /* B. UI Elements (Photos, Titles, Cards): These just need break-avoid. 
       Adding width:100% here ruins the layout, so we strictly use avoid. */
...
export default function Print({ paperRef, paper, fullName, toast }) {
  ...
  const defaultFilename = (fullName || 'My_CV').replace(/\s+/g, '_').replace(/[^\w\-_.]/g, '') + '_CV';
