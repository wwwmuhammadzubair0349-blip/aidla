...
const CANONICAL_URL = "https://www.aidla.online/tools/ai/paraphraser";
const OG_IMAGE = "https://www.aidla.online/og-paraphraser.jpg";
...
export default function AIParaphraser() {
  ...
  return (
    <>
<Helmet>
  <title>Free AI Paraphraser — Rewrite Text in Any Style | AIDLA</title>
  <meta
    name="description"
    content="Free AI-powered text paraphraser. Rewrite any text in formal, academic, casual, creative or simplified style. Powered by Groq AI. 3 free uses — no sign-up needed."
  />
  <meta
    name="keywords"
    content="AI paraphraser free, rewrite text online, paraphrase tool, text rewriter, rephrase generator, paraphrasing tool Pakistan, essay rewriter"
  />
  ...
  <meta property="og:title" content="Free AI Paraphraser — Rewrite Text in Any Style | AIDLA" />
  <meta
    property="og:description"
    content="Free AI-powered text paraphraser. Rewrite text in formal, academic, casual, creative or simplified style instantly."
  />
  <meta property="og:type" content="website" />
  <meta property="og:url" content={CANONICAL_URL} />
  <meta property="og:image" content={OG_IMAGE} />
  <meta property="og:image:alt" content="AIDLA AI Paraphraser Tool" />
  <meta property="og:site_name" content="AIDLA" />

  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:title" content="Free AI Paraphraser — Rewrite Text in Any Style | AIDLA" />
  <meta
    name="twitter:description"
    content="Rewrite text instantly with AIDLA's free AI paraphraser in formal, academic, casual, creative or simplified style."
  />
  ...
  <script type="application/ld+json">
    {JSON.stringify({
      ...
      operatingSystem: "Any",
      description:
        "Free AI paraphrasing tool to rewrite text in formal, academic, casual, creative or simplified style.",
      publisher: {
        "@type": "Organization",
        name: "AIDLA",
      ...
      <div style={S.page}>
        <div style={{ background:"#fff", borderBottom:"1px solid #f1f5f9", padding:"10px clamp(14px,4vw,24px)" }}>
          <div style={{ maxWidth:900, margin:"0 auto", fontSize:"clamp(10px,2.5vw,12px)", color:"#94a3b8", display:"flex", gap:6, flexWrap:"wrap" }}>
            ...
            <span style={{ color:"#0f172a", fontWeight:600 }}>AI Paraphraser</span>
          </div>
        </div>
        ...
        <div style={{ background:"linear-gradient(135deg,#0c4a6e,#0284c7)", padding:"clamp(24px,5vw,40px) clamp(14px,4vw,24px)", textAlign:"center" }}>
          ...
          <p style={{ margin:"0 0 14px", fontSize:"clamp(12px,3vw,15px)", color:"rgba(255,255,255,0.75)" }}>
            Powered by Groq AI · Rewrite in any style instantly
          </p>
          {!user && (
        ...
        <div style={S.wrap}>
          {/* Login gate */}
          {!canUse && !user && (
            <div style={{ background:"rgba(2,132,199,.07)", border:"1px solid rgba(2,132,199,.25)", borderRadius:16, padding:"clamp(16px,4vw,24px)", marginBottom:16, textAlign:"center" }}>
              ...
              <p style={{ color:"#64748b", fontSize:"clamp(12px,3vw,14px)", marginBottom:16, lineHeight:1.6 }}>
                Create a free AIDLA account for unlimited paraphrasing.
              </p>
              <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
                <button onClick={()=>navigate("/login?redirect=/tools/ai/paraphraser")}
                  style={{ padding:"11px 24px", background:"linear-gradient(135deg,#0284c7,#3b82f6)", color:"#fff", border:"none", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer" }}>
                  🔑 Login
                </button>
                <button onClick={()=>navigate("/signup?redirect=/tools/ai/paraphraser")}
                  style={{ padding:"11px 24px", background:"rgba(2,132,199,.1)", color:"#0284c7", border:"1px solid rgba(2,132,199,.25)", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer" }}>
                  ✨ Sign Up Free
                </button>
              </div>
            </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:16, alignItems:"start" }}>
            ...
            <div>
              <div style={S.card}>
                <span style={S.label}>Original Text</span>
              ...
              {/* Style selector */}
              <div style={S.card}>
                <span style={S.label}>Rewriting Style</span>
              ...
              <button style={S.btn} onClick={handleParaphrase} disabled={loading || (!canUse && !user)}>
          ...
          {!user && remaining > 0 && (
            <div style={{ marginTop:16, background:"rgba(2,132,199,.06)", border:"1px solid rgba(2,132,199,.15)", borderRadius:12, padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
              <div style={{ fontSize:"clamp(12px,3vw,13px)", color:"#0c4a6e" }}>
