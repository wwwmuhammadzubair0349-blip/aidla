...
function useSEO(title) {
  useEffect(() => {
    ...
    // Dashboard is protected — tell robots not to index
...
function MaterialRow({ item, onDownload, downloading }) {
  ...
  return (
    <div style={{
      background:"#fff", border:"1px solid #f1f5f9", borderRadius:13,
    ...
    >
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", flexWrap:"wrap" }}>
        ...
        <div style={{ position:"relative", flexShrink:0 }}>
          <div style={{ width:42, height:42, borderRadius:10, background:ft.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20 }}>
            {ft.icon}
          </div>
          <div style={{ position:"absolute", bottom:-2, right:-2, width:14, height:14, borderRadius:"50%", background:accentColor, border:"2px solid #fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:7 }}>
            {cat?.icon}
          </div>
        </div>
        ...
        <div style={{ flex:1, minWidth:160 }}>
          <Link to={`/resources/${item.slug}`}
            style={{ fontSize:14, fontWeight:700, color:"#0f172a", textDecoration:"none", lineHeight:1.3, display:"block",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0, flexWrap:"wrap" }}>
          ...
          {item.file_type && (
            <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:20,
              background:ft.bg, color:ft.color, textTransform:"uppercase" }}>
              {item.file_type}
            </span>
          )}
          <button
            ...
            style={{
              ...
              color: downloading === item.id ? "#15803d" : "#fff",
              border:"none", borderRadius:8,
              fontSize:12, fontWeight:700, cursor:"pointer",
              transition:"all 0.15s", whiteSpace:"nowrap",
            }}>
            {downloading === item.id ? "✅ Done!" :
              isVideo ? "▶ Watch" :
              isExternal ? "🔗 Open" :
              "⬇ Download"}
          </button>
...
export default function UserResources() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      if (!data?.user) {
       ...
       navigate(`/login?redirect=/UserResources`);
        return;
      }
      setUser(data.user);
  ...
  useEffect(() => {
    if (!checkingAuth && user) { load(); loadStats(); }
  }, [checkingAuth, user]);
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:"#f8fafc", fontFamily:"'DM Sans',sans-serif" }}>
          ...
          <div style={{ maxWidth:1100, margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
              <div>
                ...
                <h1 style={{ margin:"0 0 4px", fontSize:"clamp(1.3rem,4vw,1.8rem)", fontWeight:900, color:"#fff" }}>
                  {userName}
                </h1>
              ...
              <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                ...
                <button onClick={handleSignOut}
                  style={{ padding:"8px 16px", background:"rgba(255,255,255,0.08)", color:"rgba(255,255,255,0.7)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer" }}>
                  Sign Out
                </button>
              </div>
            </div>
        ...
        <div style={{ maxWidth:1100, margin:"0 auto", padding:"22px 14px 48px" }}>
          ...
          {/* Category pills */}
          <div className="dash-cat-pills" style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:18 }}>
            {CATEGORIES.map(c => (
              <button key={c.value} onClick={() => handleCategory(c.value)}
                style={{
                  padding:"6px 13px", borderRadius:24, fontSize:12, fontWeight:700, cursor:"pointer", border:"1px solid",
          ...
          {totalPages > 1 && !loading && (
            <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:6, marginTop:28, flexWrap:"wrap" }}>
              <button
                onClick={() => { const p = Math.max(1,page-1); setPage(p); load(search,category,p); }}
                disabled={page===1}
              ...
              {Array.from({length:Math.min(totalPages,7)},(_,i)=>i+1).map(p=>(
                <button key={p}
                  onClick={() => { setPage(p); load(search,category,p); }}
                  style={{
                    width:34, height:34, border:"1px solid", borderRadius:8,
              ...
              <button
                onClick={() => { const p = Math.min(totalPages,page+1); setPage(p); load(search,category,p); }}
