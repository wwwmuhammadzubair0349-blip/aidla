...
export default function Invite() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (error && error.code === "PGRST116") {
        ...
        const { error: insErr } = await supabase.from("users_profiles").insert([{ user_id: uid, email, full_name: fullName }]);
        if (insErr) { setMsg(insErr.message); setLoading(false); return; }
        const res2 = await supabase.from("users_profiles").select("my_refer_code, my_referals").eq("user_id", uid).single();
      ...
      if (!prof?.my_refer_code) {
        await supabase.from("users_profiles").update({ my_refer_code: null }).eq("user_id", uid);
        const { data: prof2, error: e2 } = await supabase.from("users_profiles").select("my_refer_code, my_referals").eq("user_id", uid).single();
  ...
  // Load referrals: find all users whose referral_code_used = my code
  useEffect(() => {
    if (!code) return;
    (async () => {
      setRefLoading(true);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: "Join AIDLA 🚀",
          text: `Join AIDLA with my invite code: ${code}`,
          url: inviteLink,
        });
      } catch (e) {
    ...
    <div className="invite-wrapper">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* ── Code Card with Share ── */}
          <div className="code-card">
            <div className="code-label">Your Invite Code</div>
            <div className="code-value">{code || "—"}</div>
            <div className="code-actions">
              <button
                className={`code-btn btn-white${copied === "code" ? " done" : ""}`}
              ...
              <button
                className="code-btn btn-outline"
                onClick={nativeShare}
                disabled={!code}
              >
                📤 Share Now
          ...
          <div className="link-card">
            ...
            <div className="link-actions">
              <button
                className={`action-btn btn-primary${copied === "link" ? " done" : ""}`}
              ...
              <button
                className={`action-btn btn-secondary${copied === "msg" ? " done" : ""}`}
          ...
          <div className="ref-section">
            ...
            {refLoading ? (
              <div className="loader-wrap" style={{ padding: "20px" }}>
                <div className="spinner" />
                Loading referrals…
              </div>
            ) : referrals.length === 0 ? (
