...
// ─────────────────────────────────────────────────────────
// ⛏️ MINING VISUAL — clean, professional, no animations
// ─────────────────────────────────────────────────────────
    ...
    <div style={{ padding: "28px 24px 24px", background: statusBg, borderBottom: `1px solid ${statusBorder}`, transition: "background 0.4s" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
        <div style={{ position: "relative", width: 10, height: 10 }}>
          ...
          {isMining && !isDone && (
            <div style={{
              position: "absolute", inset: 0, borderRadius: "50%", background: statusColor, opacity: 0.3,
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
          <div style={{ position: "relative", width: 110, height: 110 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: "1.1rem", fontWeight: 800, color: statusColor, letterSpacing: "-0.02em" }}>
                {Math.floor(pct)}%
              </span>
              <span style={{ fontSize: "0.58rem", color: "#94a3b8", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                complete
              </span>
            </div>
          </div>
      ...
      {!isMining && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
          <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#f1f5f9", border: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem" }}>
            ⛏️
          </div>
        </div>
      )}
...
// ─────────────────────────────────────────────────────────
function ConfirmModal({ booster, balance, onConfirm, onCancel, buying }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => { const t = setTimeout(() => setVisible(true), 30); return () => clearTimeout(t); }, []);
  const after = balance - booster.price_coins;
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: "#fff", borderRadius: 24, padding: "28px 24px", width: "100%", maxWidth: 340,
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
          <div style={{ width: 60, height: 60, borderRadius: 18, background: "#eff6ff", border: "1px solid #bfdbfe", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.8rem" }}>
            {booster.icon || "⚡"}
          </div>
        </div>
        {/* Title */}
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <div style={{ fontSize: "0.78rem", color: "#64748b" }}>
            This will activate a <strong>{booster.multiplier}×</strong> mining speed boost
            {booster.duration_hours ? ` for ${booster.duration_hours} hours` : " permanently"}.
          </div>
        </div>
        {/* Cost breakdown */}
        <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 14, padding: "14px 16px", marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", marginBottom: 8 }}>
            ...
            <span style={{ fontWeight: 700, color: "#0f172a" }}>💰 {fmtS(balance)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem", marginBottom: 10 }}>
            <span style={{ color: "#64748b" }}>Booster cost</span>
            <span style={{ fontWeight: 700, color: "#dc2626" }}>− {booster.price_coins}</span>
          </div>
          <div style={{ borderTop: "1px dashed #e2e8f0", paddingTop: 10, display: "flex", justifyContent: "space-between", fontSize: "0.82rem" }}>
            <span style={{ color: "#64748b", fontWeight: 600 }}>After purchase</span>
            <span style={{ fontWeight: 800, color: after >= 0 ? "#16a34a" : "#dc2626" }}>💰 {fmtS(after)}</span>
          </div>
        </div>
        {/* Actions */}
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onCancel} disabled={buying} style={{
            flex: 1, padding: "12px", borderRadius: 12, border: "1px solid #e2e8f0",
            background: "#f8fafc", color: "#334155", fontWeight: 700, fontSize: "0.88rem",
            cursor: "pointer", opacity: buying ? 0.5 : 1
          }}>
            Cancel
          </button>
          <button onClick={onConfirm} disabled={buying} style={{
            flex: 2, padding: "12px", borderRadius: 12, border: "none",
            background: buying ? "#93c5fd" : "linear-gradient(135deg,#2563eb,#0ea5e9)",
...
// ─────────────────────────────────────────────────────────
// ✅ SUCCESS TOAST — top-center slide down
// ─────────────────────────────────────────────────────────
function SuccessToast({ data, onDone }) {
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    const t1 = setTimeout(() => setVisible(true), 30);
    const t2 = setTimeout(() => { setVisible(false); setTimeout(onDone, 400); }, 3500);
  ...
  return (
    <div style={{
      ...
      zIndex: 99999, width: "calc(100% - 32px)", maxWidth: 380,
      pointerEvents: "none"
    }}>
      <div style={{
        ...
        <div style={{ width: 44, height: 44, borderRadius: 12, background: "#f0fdf4", border: "1px solid #bbf7d0", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.3rem", flexShrink: 0 }}>
          {data?.icon || "⚡"}
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: "0.75rem", fontWeight: 700, color: "#16a34a", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 2 }}>
            Booster Activated!
          </div>
          <div style={{ fontSize: "0.88rem", fontWeight: 700, color: "#0f172a" }}>
            {data?.name} — {data?.multiplier}× speed
          </div>
          <div style={{ fontSize: "0.68rem", color: "#64748b", marginTop: 2 }}>
            Mining rate multiplier applied to your session
          </div>
        </div>
        <div style={{ fontSize: "1.2rem" }}>✅</div>
      </div>
      {/* Progress bar */}
      <div style={{ height: 3, background: "rgba(0,0,0,0.06)", borderRadius: "0 0 16px 16px", overflow: "hidden", marginTop: -1 }}>
        <div style={{ height: "100%", background: "#16a34a", borderRadius: "0 0 16px 16px", animation: visible ? "shrinkBar 3.5s linear forwards" : "none" }} />
      </div>
...
export default function Mining() {
  ...
  const [confirmBooster,setConfirmBooster]= useState(null); // booster object pending confirmation
  const [buySuccess,    setBuySuccess]    = useState(null); // { name, icon, multiplier }
  ...
  const confirmPurchase = async () => {
    const b = confirmBooster;
    setConfirmBooster(null);
    setBuying(b.id);
      ...
      {confirmBooster && mounted && createPortal(
        <div onClick={() => setConfirmBooster(null)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 99999, padding: "0 24px" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 24, padding: "28px 24px", textAlign: "center", width: "100%", maxWidth: 320, boxShadow: "0 24px 60px rgba(0,0,0,0.15)" }}>
            <div style={{ width: 56, height: 56, borderRadius: 16, background: "#eff6ff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.6rem", margin: "0 auto 16px" }}>
              {confirmBooster.icon || "⚡"}
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmBooster(null)} style={{ flex: 1, padding: "12px", borderRadius: 12, border: "1px solid #e2e8f0", background: "#fff", fontWeight: 600, fontSize: "0.88rem", color: "#64748b", cursor: "pointer" }}>
                Cancel
              </button>
              <button onClick={confirmPurchase} style={{ flex: 2, padding: "12px", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#2563eb,#0ea5e9)", color: "#fff", fontWeight: 700, fontSize: "0.88rem", cursor: "pointer", boxShadow: "0 4px 0 #1e40af" }}>
                Confirm Buy
              </button>
            </div>
          </div>
      ...
      {buySuccess && mounted && createPortal(
        <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", zIndex: 99998, pointerEvents: "none", width: "calc(100% - 32px)", maxWidth: 400 }}>
          <div style={{ background: "#fff", border: "1px solid #bbf7d0", borderRadius: 16, padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "#f0fdf4", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", flexShrink: 0 }}>
              {buySuccess.icon || "⚡"}
            </div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ padding: "18px 16px" }}>
                {!hasActive ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    {/* Claim button */}
                    {isDone && (
                      <Btn variant="claim" onClick={onClaim} disabled={claiming}>
                        {claiming ? "Processing…" : `🎁  Claim ${fmtS(sd?.earned ?? 0)} coins`}
                      </Btn>
                    )}
                  </div>
                )}
                  ...
                  {boosters.map(b => {
                    ...
                    return (
                      <div key={b.id} style={{ background: owned ? "#f0fdf4" : "#f8fafc", border: `1px solid ${owned ? "#86efac" : "#e2e8f0"}`, borderRadius: 16, padding: 14, display: "flex", flexDirection: "column", gap: 8, opacity: (!owned && !canAfford) ? 0.7 : 1 }}>
                        <div style={{ width: 40, height: 40, borderRadius: 12, background: owned ? "#dcfce7" : "#eff6ff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem" }}>
                          {b.icon || "⚡"}
                        </div>
