// Certificate.jsx — AIDLA Branded Certificate (Advanced Edition)
// Features: PNG/PDF export, smart social sharing, share modal, share tracking
// Fixes: SVG seal (html2canvas safe), mobile toolbar (CSS grid, zero overflow)
...
// ─── SVG seal — renders correctly in html2canvas & jsPDF ─────────────────────
const SEAL_SVG = `
...
<div style="width:100%;height:100%;background:#FFFEF9;position:relative;overflow:hidden;font-family:'Plus Jakarta Sans',sans-serif;">
  ...
  <div style="position:relative;z-index:3;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:130px 130px 96px;">
    ...
    <div style="display:flex;align-items:center;gap:10px;font-size:9px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:22px;">
      ${courseLevel?`<span>${courseLevel}</span>`:''}
      ${courseLevel&&courseCategory?'<div style="width:3px;height:3px;border-radius:50%;background:#E8EDF5;"></div>':''}
      ${courseCategory?`<span>${courseCategory}</span>`:''}
    </div>`:''}
    <div style="display:flex;align-items:flex-end;justify-content:space-between;width:100%;gap:20px;margin-top:auto;">
      ...
      <div style="display:flex;flex-direction:column;align-items:center;flex:1;">
        <div style="font-family:'Instrument Serif',Georgia,serif;font-style:italic;font-size:22px;color:#003A8C;border-bottom:1.5px solid #CBD5E1;padding-bottom:4px;min-width:160px;text-align:center;margin-bottom:5px;">${issued}</div>
  ...
  <div style="position:absolute;bottom:52px;right:76px;z-index:4;display:flex;flex-direction:column;align-items:center;gap:5px;">
    <img src="${qrUrl}" width="86" height="86" alt="QR" style="border-radius:4px;display:block;">
  ...
  <div style="position:absolute;bottom:52px;left:76px;font-size:8px;color:#94A3B8;font-family:monospace;line-height:1.9;z-index:3;">
    Certificate No: ${certNumber}<br>Verify: ${verifyUrl}
  </div>
</div>`;
}
...
const PLATFORMS = [
  { id:'linkedin',  label:'LinkedIn',    bg:'#0077B5', color:'#fff',
    icon:<svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg> },
  { id:'twitter',   label:'Twitter / X', bg:'#000',    color:'#fff',
    icon:<svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.259 5.63L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z"/></svg> },
  { id:'whatsapp',  label:'WhatsApp',    bg:'#25D366', color:'#fff',
    icon:<svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg> },
  { id:'facebook',  label:'Facebook',    bg:'#1877F2', color:'#fff',
    icon:<svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
  { id:'instagram', label:'Instagram',   bg:'linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)', color:'#fff',
    icon:<svg viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg> },
];
...
function ShareModal({ cert, verifyUrl, certHTML, onClose }) {
  ...
  useEffect(() => {
    setCaption(buildCaption({ studentName:cert.student_name, courseTitle:cert.course_title, verifyUrl, platform:active }));
  }, [active]);

  useEffect(() => {
    (async () => {
      setRender(true);
      try { const c = await renderCertToCanvas(certHTML); setImg(c.toDataURL('image/png')); }
  ...
  const doShare = async (pid) => {
    ...
    const eu = encodeURIComponent(verifyUrl);
    ...
    setStatus(s => ({ ...s, [pid]:'done' }));
    setTimeout(() => setStatus(s => ({ ...s, [pid]:undefined })), 3000);
  };

  const p  = PLATFORMS.find(pl => pl.id === active);
  const st = status[active];

  return (
    <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(26,26,46,0.72)', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}
      ...
      <div style={{ background:C.white, borderRadius:18, width:'100%', maxWidth:600, boxShadow:'0 28px 72px rgba(0,0,0,0.22)', overflow:'hidden', maxHeight:'92vh', display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'20px 24px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <div style={{ fontWeight:800, fontSize:17, color:C.ink }}>🎉 Share Your Achievement</div>
            <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>Certificate image auto-downloads for image-based platforms</div>
          </div>
          <button onClick={onClose} style={{ background:C.bg, border:'none', borderRadius:8, width:34, height:34, cursor:'pointer', fontSize:18, color:C.slate, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
        </div>
        <div style={{ padding:'20px 24px', overflowY:'auto', flex:1 }}>
          <div style={{ width:'100%', height:148, borderRadius:10, overflow:'hidden', marginBottom:18, border:`1px solid ${C.border}`, background:'#FFFEF9', display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
            {rendering
              ? <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8 }}>
                  <div style={{ width:24, height:24, border:`3px solid ${C.border}`, borderTopColor:C.blue, borderRadius:'50%', animation:'spin .8s linear infinite' }}/>
                  <div style={{ fontSize:11, color:C.muted }}>Rendering certificate…</div>
                </div>
              : img
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              {PLATFORMS.map(pl => (
                <button key={pl.id} onClick={() => setActive(pl.id)} style={{
                  ...
                  background: active===pl.id ? pl.bg : C.bg,
                  backgroundImage: active===pl.id && pl.bg.includes('gradient') ? pl.bg : undefined,
                  color: active===pl.id ? pl.color : C.slate,
                  outline: active===pl.id ? `2px solid ${C.blue}` : 'none', outlineOffset:2, transition:'all .12s',
                }}>{pl.icon} {pl.label}</button>
              ))}
            </div>
          </div>
          {(active==='linkedin'||active==='instagram') && (
            <div style={{ background:active==='linkedin'?'#EFF6FF':'#FDF4FF', border:`1px solid ${active==='linkedin'?'#BFDBFE':'#E9D5FF'}`, borderRadius:8, padding:'9px 13px', marginBottom:14, fontSize:11.5, color:active==='linkedin'?'#1E40AF':'#6B21A8', lineHeight:1.6 }}>
              <strong>How it works:</strong> {active==='linkedin'
                ? 'PNG downloads + caption copied. LinkedIn will open — paste caption and attach the image. 🎯'
                : 'PNG downloads + caption copied. Instagram will open — create a post and paste. 📸'}
            </div>
          )}
          <div style={{ marginBottom:18 }}>
            ...
            <textarea value={caption} onChange={e=>setCaption(e.target.value)} rows={6}
              style={{ width:'100%', border:`1.5px solid ${C.border}`, borderRadius:9, padding:'11px 13px', fontSize:12.5, lineHeight:1.65, color:C.ink, resize:'vertical', fontFamily:'inherit', background:C.bg, outline:'none' }}
              onFocus={e=>e.target.style.borderColor=C.blue} onBlur={e=>e.target.style.borderColor=C.border}/>
          </div>
          <button onClick={() => doShare(active)} disabled={rendering} style={{
            width:'100%', padding:'12px 20px', borderRadius:9, border:'none', cursor:rendering?'not-allowed':'pointer',
            fontWeight:800, fontSize:13.5, background:st==='done'?C.success:p.bg,
            backgroundImage: st!=='done'&&p.bg.includes('gradient')?p.bg:undefined,
            color:'#fff', boxShadow:'0 4px 14px rgba(0,0,0,0.15)',
            display:'flex', alignItems:'center', justifyContent:'center', gap:8, opacity:rendering?0.6:1, transition:'all .2s',
          }}>
            {st==='loading'
              ? <><div style={{ width:15, height:15, border:'2.5px solid rgba(255,255,255,0.3)', borderTopColor:'#fff', borderRadius:'50%', animation:'spin .8s linear infinite' }}/> Preparing…</>
...
export default function Certificate() {
  ...
  const load = async () => {
    try {
      ...
      if (!name) {
        ...
        name = session?.user?.user_metadata?.full_name || session?.user?.email?.split('@')[0] || 'A Dedicated Learner';
      }
  ...
  const downloadPNG = async () => {
    if (!cert) return; setExport('png');
    try { const c = await renderCertToCanvas(certHTML); const a=document.createElement('a'); a.href=c.toDataURL('image/png'); a.download=`AIDLA-Certificate-${cert.certificate_number}.png`; a.click(); }
    catch(e) { alert('PNG export failed.'); }
    setExport(null);
  };

  const downloadPDF = async () => {
    if (!cert) return; setExport('pdf');
    try {
      const [canvas, jsPDF] = await Promise.all([renderCertToCanvas(certHTML), loadPDF()]);
      const pdf = new jsPDF({ orientation:'landscape', unit:'mm', format:'a4' });
      pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, 0, 297, 210);
      pdf.save(`AIDLA-Certificate-${cert.certificate_number}.pdf`);
  ...
  const printCert = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>AIDLA Certificate</title>
...
<style>@page{size:A4 landscape;margin:0}*{box-sizing:border-box;margin:0;padding:0}html,body{width:100vw;height:100vh;background:#FFFEF9;overflow:hidden;display:flex;align-items:center;justify-content:center;-webkit-print-color-adjust:exact;print-color-adjust:exact}#pw{width:980px;height:693px;transform-origin:center;transform:scale(min(calc(100vw/980),calc(100vh/693)))}</style>
</head><body onload="setTimeout(()=>{window.print();window.close()},500)"><div id="pw">${buildCertHTML({ studentName:cert.student_name, courseTitle:cert.course_title, courseLevel:cert.course_level, courseCategory:cert.course_category, issued, certNumber:cert.certificate_number, verifyUrl })}</div></body></html>`);
    win.document.close();
  };
  ...
  if (error) return (
    <div style={{ display:'flex', height:'100vh', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:12, fontFamily:'system-ui', background:C.bg }}>
      ...
      <Link to="/user/courses" style={{ background:C.blue, color:C.white, padding:'10px 22px', borderRadius:8, textDecoration:'none', fontWeight:700 }}>← Back to Courses</Link>
    </div>
  );
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .tb-back{
          ...
          border:1.5px solid ${C.blue}!important;
          background:${C.white};color:${C.blue};flex-shrink:0;
        }
        ...
        .tb-copy{
          ...
          background:${C.white};color:${C.slate};flex-shrink:0;
        }
        ...
        /* ── Info strip ── */
        .info-strip{
          background:${C.white};border-radius:10px;margin-top:12px;
          padding:12px 18px;display:flex;align-items:center;gap:12px;
          box-shadow:0 2px 8px rgba(0,0,0,.04);border:1px solid ${C.border};
      ...
      <div className="cert-page">
        ...
        <div className="toolbar">
          <Link to="/user/courses" className="tb-el tb-back">← Dashboard</Link>
          <div className="tb-div"/>
          <div className="btn-group">
            <button className="tb-exp" onClick={downloadPNG} disabled={!!exporting}>
              {exporting==='png' ? <><Spin/> PNG…</> : '⬇ PNG'}
            </button>
            <button className="tb-exp" onClick={downloadPDF} disabled={!!exporting}>
              {exporting==='pdf' ? <><Spin/> PDF…</> : '⬇ PDF'}
            </button>
            <button className="tb-exp" onClick={printCert}>🖨 Print</button>
          </div>
          <button className={`tb-el tb-copy${copied?' tb-copy-done':''}`} onClick={copyLink}>
            {copied ? '✓ Copied!' : '🔗 Copy Link'}
          </button>
          <button className="tb-el tb-share" onClick={() => setShare(true)}>
            🚀 Share Certificate
          </button>
        </div>
        ...
        <div className="cert-wrap" ref={wrapRef}>
          <div className="cert-scaler" style={{ transform:`scale(${scale})` }}
      ...
      {showShare && (
        <ShareModal cert={cert} verifyUrl={verifyUrl} certHTML={certHTML} onClose={() => setShare(false)}/>
      )}
    </>
  );
}